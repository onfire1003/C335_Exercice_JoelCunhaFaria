...
import com.example.minidepences.data.AppDatabase
...
fun StatisticsScreen(
    ...
    Column(
        ...
        val stats = listOf(
            CategoryStat(ExpenseCategories.FOOD, foodTotal ?: 0.0, foodCount),
            CategoryStat(ExpenseCategories.TRANSPORT, transportTotal ?: 0.0, transportCount),
            CategoryStat(ExpenseCategories.LEISURE, leisureTotal ?: 0.0, leisureCount),
            CategoryStat(ExpenseCategories.HEALTH, healthTotal ?: 0.0, healthCount),
            CategoryStat(ExpenseCategories.OTHER, otherTotal ?: 0.0, otherCount),
        ).filter { it.total > 0.0 }
        ...
        CategoryStatItem(title = ExpenseCategories.FOOD, count = foodCount, total = foodTotal)
        Spacer(modifier = Modifier.height(8.dp))
        CategoryStatItem(title = ExpenseCategories.TRANSPORT, count = transportCount, total = transportTotal)
        Spacer(modifier = Modifier.height(8.dp))
        CategoryStatItem(title = ExpenseCategories.LEISURE, count = leisureCount, total = leisureTotal)
        Spacer(modifier = Modifier.height(8.dp))
        CategoryStatItem(title = ExpenseCategories.HEALTH, count = healthCount, total = healthTotal)
        Spacer(modifier = Modifier.height(8.dp))
        CategoryStatItem(title = ExpenseCategories.OTHER, count = otherCount, total = otherTotal)
